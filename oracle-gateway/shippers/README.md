...
Each laptop needs two things, and nothing else:

```bash
export ORACLE_GATEWAY_URL=http://SERVER:8080
export ORACLE_TOKEN=ork_...            # your own token, not shared
```
...
for f in $(find ~/.claude/projects -name '*.jsonl' -mtime -1); do
  curl -s -H "Authorization: Bearer $ORACLE_TOKEN" \
...
```bash
curl -H "Authorization: Bearer $ORACLE_TOKEN" -H 'Content-Type: application/json' \
